...
# Parse data from ResearchResults.typ
data = []
with open('ResearchResults.typ', 'r', encoding='utf-8') as f:
    for line in f:
        # Match lines that look like: ..., [N], [N], [N], [P],
        match = re.search(r'\[(P|N|ND)\],\s*\[(P|N|ND)\],\s*\[(P|N|ND)\],\s*\[(P|N|ND)\]', line)
        if match:
...
plt.title(f'Rozkład ogólnej liczby ocen (Suma {len(data)*4} ocen)', fontsize=12)
...
plt.title(f'Zgodność ewaluatorów w obrębie {len(data)} pytań', fontsize=12)
